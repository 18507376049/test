...
import com.logisticspro.exception.NoBalanceException;
import com.logisticspro.pojo.*;
import com.logisticspro.service.inter.AuthorityManagerServiceInter;
import com.logisticspro.util.CommandMethod;
import com.logisticspro.util.LayuiTreeAuthority;
import com.logisticspro.pojo.Page;
...
public class AuthorityManagerServiceImpl implements AuthorityManagerServiceInter {
    ...
    public int excuteAddRoles(Role role, MaxNo maxNo) {
        ...
        //设置值 要修改的编号
        maxNo.setManMaxNo(role.getRolNo());
        int rows = 0;
        int i1 = 0;
        //修改最大编号
        try {
            rows = commandMethod.updateMax(maxNo);
    ...
    public int insertAuthority(Authority authority, MaxNo maxNo) {
        //设置值 要修改的编号
        maxNo.setManMaxNo(authority.getAutNo());
    ...
    public int setAuthority(List<LayuiTreeAuthority<Employees>> employees, List<LayuiTreeAuthority<Authority>> authority, List<LayuiTreeAuthority<Role>> role) {
        ...
        for (LayuiTreeAuthority<Employees> employeesLayuiTree : employees) {
            ...
            maxNo.setManTableName("employees_role");
            //2.1 获得员工角色最大编号
            String puMaxNo = commandMethod.getPuMaxNo(maxNo);
            maxNo.setManMaxNo(puMaxNo);
